...
def main():
    ...
    n.section("Overview")
    n.write(
        "This report analyzes **VCB (Vietcombank)** daily trading data "
    ...
    n.summary(df[["close", "volume", "high", "low"]], title="VCB Trading Data Summary")
    ...
    if HAS_MPL:
        ...
        ax.set_title("VCB Daily Close Price with High-Low Band")
        ax.set_xlabel("Date")
        ...
        fig.autofmt_xdate()
        fig.tight_layout()
        n.figure(fig, "vcb_price_band.png", caption="VCB daily closing price with high-low range")
        plt.close(fig)
        ...
        fig.autofmt_xdate()
        fig.tight_layout()
        n.figure(fig, "vcb_volume.png", caption="Green = close >= open, Red = close < open")
        plt.close(fig)

    # ── Export ──
    n.section("Data Export")
    n.export_csv(df, "vcb_prices.csv", name="VCB price data")
    n.export_csv(weekly, "vcb_weekly.csv", name="VCB weekly aggregation")
    ...
    n.write(
        f"""
- VCB shows a **steady upward trend** over the sample period, gaining **{total_return:.1f}%**.
